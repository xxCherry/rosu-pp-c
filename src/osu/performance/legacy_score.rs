...
pub fn calculate_legacy_score_miss_count(
    state: &OsuScoreState,
    acc: f64,
    mods: &GameMods,
    ...
    let score_v1_multiplier = get_legacy_score_multiplier(mods);
    let relevant_combo_per_object = calculate_relevant_score_combo_per_object(attributes);

    let maximum_miss_count = calculate_maximum_combo_based_miss_count(state, attributes);

    let score_obtained_during_max_combo = calculate_score_at_combo(
        state,
        acc,
        state.max_combo,
    ...
    let remaining_combo = attributes.max_combo - state.max_combo;
    let expected_remaining_score = calculate_score_at_combo(
        state,
        acc,
        remaining_combo,
        relevant_combo_per_object,
...
fn calculate_score_at_combo(
    ...
    let objects_hit =
        f64::from(total_hits - state.misses) * f64::from(combo) / f64::from(attributes.max_combo);

    // * Score also has a non-combo portion we need to create the final score value.
    let non_combo_score = (300.0 + attributes.nested_score_per_object) * acc * objects_hit;
...
fn calculate_relevant_score_combo_per_object(attributes: &OsuDifficultyAttributes) -> f64 {
    ...
    let mut result = (attributes.max_combo as f64 - 2.0) * attributes.max_combo as f64;
    result /= (attributes.max_combo as f64 + 2.0 * (combo_score - 1.0)).max(1.0);
...
fn calculate_maximum_combo_based_miss_count(
    ...
    let full_combo_threshold =
        f64::from(attributes.max_combo) - 0.1 * f64::from(attributes.n_circles);
    ...
    let max_possible_slider_breaks =
        ((attributes.max_combo - state.max_combo) / 2).min(attributes.n_sliders);
...
fn get_legacy_score_multiplier(mods: &GameMods) -> f64 {
